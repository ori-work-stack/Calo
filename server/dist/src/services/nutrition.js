...
class NutritionService {
    static async analyzeMeal(user_id, data) {
        ...
        const needsReset = now.getDate() !== resetTime.getDate() ||
        ...
        const permissions = await auth_1.AuthService.getRolePermissions(user.subscription_type);
        if (permissions.dailyRequests !== -1 &&
            user.aiRequestsCount >= permissions.dailyRequests) {
            throw new Error("Daily AI request limit reached.");
        }
        try {
            const analysis = await openai_1.OpenAIService.analyzeMealImage(cleanBase64, language);
            await database_1.prisma.user.update({
                where: { user_id: user_id },
                data: {
    ...
    static async updateMeal(user_id, data) {
        ...
        try {
            ...
            const updatedAnalysis = await openai_1.OpenAIService.analyzeMealImage(imageBase64, language, updateText);
            ...
            const updatedMeal = await database_1.prisma.meal.update({
    ...
    static async saveMeal(user_id, mealData, imageBase64) {
        try {
            console.log("💾 Saving meal for user:", user_id);
            const calories = typeof mealData.calories === "number"
                ? mealData.calories
                : parseFloat(mealData.totalCalories) || 0;
            const protein = typeof mealData.protein === "number"
                ? mealData.protein
                : parseFloat(mealData.totalProtein) || 0;
            const carbs = typeof mealData.carbs === "number"
                ? mealData.carbs
                : parseFloat(mealData.totalCarbs) || 0;
            const fat = typeof mealData.fat === "number"
                ? mealData.fat
                : parseFloat(mealData.totalFat) || 0;
            const fiber = typeof mealData.fiber === "number"
                ? mealData.fiber
                : parseFloat(mealData.totalFiber || "0") || 0;
            const sugar = typeof mealData.sugar === "number"
                ? mealData.sugar
                : parseFloat(mealData.totalSugar || "0") || 0;
            const meal = await database_1.prisma.meal.create({
                data: {
                    user_id: user_id,
    ...
    static async saveMealFeedback(user_id, meal_id, feedback) {
        try {
            ...
            const existingAdditives = meal.additives_json || {};
    ...
    static async toggleMealFavorite(user_id, meal_id) {
        try {
    ...
    static async duplicateMeal(user_id, meal_id, newDate) {
        try {
            console.log("📋 Duplicating meal:", meal_id, "for user:", user_id);
            // Find the original meal
            const originalMeal = await database_1.prisma.meal.findFirst({
                where: {
                    meal_id: parseInt(meal_id),
                    user_id: user_id,
                },
            ...
            const duplicatedMeal = await database_1.prisma.meal.create({
                data: {
            ...
            console.log("✅ Meal duplicated successfully with ID:", duplicatedMeal.meal_id);
            // Transform the meal data to match client expectations
            const transformedMeal = {
                // Server fields
