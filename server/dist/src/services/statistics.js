...
class StatisticsService {
    static async getNutritionStatistics(userId, period) {
        try {
            console.log(`📊 Getting nutrition statistics for user: ${userId}, period: ${period}`);
    ...
    static calculateStatistics(meals, dailyCalorieGoal) {
        if (meals.length === 0) {
        ...
        // Calculate averages
        const averageCaloriesDaily = Math.round(dailyTotals.reduce((sum, day) => sum + day.calories, 0) / totalDays);
        const averageProteinDaily = Math.round(dailyTotals.reduce((sum, day) => sum + day.protein, 0) / totalDays);
        const averageCarbsDaily = Math.round(dailyTotals.reduce((sum, day) => sum + day.carbs, 0) / totalDays);
        const averageFatsDaily = Math.round(dailyTotals.reduce((sum, day) => sum + day.fats, 0) / totalDays);
        const averageFiberDaily = Math.round(dailyTotals.reduce((sum, day) => sum + day.fiber, 0) / totalDays);
        const averageSodiumDaily = Math.round(dailyTotals.reduce((sum, day) => sum + day.sodium, 0) / totalDays);
        const averageSugarDaily = Math.round(dailyTotals.reduce((sum, day) => sum + day.sugar, 0) / totalDays);
        // Calculate calorie goal achievement
        const calorieGoalAchievementPercent = Math.round((dailyTotals.filter((day) => day.calories >= dailyCalorieGoal * 0.9 &&
            day.calories <= dailyCalorieGoal * 1.1).length /
            totalDays) *
            100);
        // Calculate processed food percentage using correct field names
        const processedFoodCount = meals.filter((meal) => meal.processing_level === "HIGHLY_PROCESSED" ||
            meal.processing_level === "PROCESSED" ||
            (meal.health_warnings && meal.health_warnings.includes("מעובד"))).length;
        const processedFoodPercentage = Math.round((processedFoodCount / meals.length) * 100);
        // Calculate full logging percentage (3+ meals per day)
        const fullLoggingDays = dailyTotals.filter((day) => day.mealCount >= 3).length;
        const fullLoggingPercentage = Math.round((fullLoggingDays / totalDays) * 100);
        // Calculate eating hours
        const eatingTimes = meals
            .map((meal) => meal.createdAt.getHours())
            .filter((hour) => hour !== null);
        const averageEatingHours = {
            start: eatingTimes.length > 0 ? `${Math.min(...eatingTimes)}:00` : "08:00",
            end: eatingTimes.length > 0 ? `${Math.max(...eatingTimes)}:00` : "20:00",
        };
        // Calculate intermittent fasting
        const fastingHours = eatingTimes.length >= 2
            ? 24 - (Math.max(...eatingTimes) - Math.min(...eatingTimes))
            : 0;
        // Calculate alcohol and caffeine intake using correct field names
        const alcoholCaffeineIntake = Math.round(meals.reduce((sum, meal) => sum + (meal.alcohol_g || 0) + (meal.caffeine_mg || 0) / 100, 0) / totalDays);
        // Calculate vegetable/fruit intake (simplified)
        const vegetableFruitIntake = Math.round((meals.filter((meal) => meal.description?.includes("vegetable") ||
            meal.description?.includes("fruit") ||
            meal.description?.includes("ירק") ||
            meal.description?.includes("פרי")).length /
            meals.length) *
            100);
        ...
        meals.forEach((meal) => {
            if (meal.allergens) {
                try {
                    const allergens = typeof meal.allergens === "string"
        ...
        const healthRiskPercentage = Math.round((healthRiskMeals / meals.length) * 100);
        ...
        const missedMealsAlert = Math.max(0, expectedMealsPerWeek - actualMealsThisWeek);
        return {
    ...
    static calculateNutritionScore(metrics) {
        let score = 0;
    ...
    static async generateAIInsights(meals, stats) {
        const insights = [];
        // Calorie insights
        if (stats.calorieGoalAchievementPercent < 50) {
            insights.push("נבחן כי קשה לך לעמוד ביעד הקלורי היומי. מומלץ לתכנן ארוחות מראש");
        }
        ...
        if (stats.processedFoodPercentage > 30) {
            insights.push("שיעור המזון המעובד שלך גבוה מהמומלץ. נסה להוסיף יותר מזונות טבעיים");
        }
        // Timing insights
        if (stats.intermittentFastingHours > 16) {
            insights.push(`הצום היומי שלך נמשך ${stats.intermittentFastingHours} שעות - זה מעולה לבריאות המטבולית`);
        }
        // Logging insights
        ...
        try {
            ...
            const aiInsights = await openai_1.OpenAIService.generateNutritionInsights(meals, stats);
    ...
    static async generateRecommendations(stats, user) {
        const recommendations = [];
        // Personalized recommendations based on statistics
        if (stats.averageFiberDaily < 25) {
            recommendations.push("הוסף יותר ירקות עלים ירוקים וקטניות לתפריט לשיפור צריכת הסיבים");
        }
        if (stats.averageSodiumDaily > 2300) {
            recommendations.push("צמצם מזונות מעובדים ותבלינים מלוחים להפחתת צריכת הנתרן");
        }
        if (stats.averageProteinDaily < (user?.weight_kg || 70) * 0.8) {
            recommendations.push("הגבר צריכת חלבון באמצעות עוף רזה, דגים, קטניות וביצים");
        }
        if (stats.processedFoodPercentage > 25) {
            recommendations.push("התמקד במזונות טבעיים ובישול בבית לשיפור איכות התזונה");
        }
        ...
        if (stats.missedMealsAlert > 5) {
            recommendations.push("שמר על קביעות בארוחות - 3 ארוחות ביום לשמירה על רמת הסוכר");
        }
    ...
    static getDefaultInsights(stats) {
        const insights = [];
    ...
    static getEmptyStatisticsWithDefaults() {
        return {
            averageCaloriesDaily: 0,
            calorieGoalAchievementPercent: 0,
            averageProteinDaily: 0,
