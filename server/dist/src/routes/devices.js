...
router.get("/", async (req, res) => {
    try {
        ...
        const devices = await devices_1.DeviceService.getUserDevices(req.user.user_id);
        res.json({
            success: true,
            data: devices,
        });
...
router.post("/connect", async (req, res) => {
    try {
        ...
        const device = await devices_1.DeviceService.connectDevice(req.user.user_id, deviceType, deviceName, accessToken, refreshToken);
        res.json({
            success: true,
            data: device,
        });
...
router.delete("/:deviceId", async (req, res) => {
    try {
        ...
        await devices_1.DeviceService.disconnectDevice(req.user.user_id, deviceId);
        res.json({
            success: true,
...
router.post("/:deviceId/sync", async (req, res) => {
    try {
        ...
        const result = await devices_1.DeviceService.syncDeviceData(req.user.user_id, deviceId, activityData);
        res.json({
            success: true,
            data: result,
        });
...
router.get("/activity/:startDate/:endDate", async (req, res) => {
    try {
        ...
        const activityData = await devices_1.DeviceService.getActivityData(req.user.user_id, startDate, endDate);
        res.json({
            success: true,
            data: activityData,
        });
...
router.get("/balance/:date", async (req, res) => {
    try {
        ...
        const balance = await devices_1.DeviceService.getDailyBalance(req.user.user_id, date);
        res.json({
            success: true,
            data: balance,
        });
        ...
        const message = error instanceof Error ? error.message : "Failed to fetch daily balance";
        res.status(500).json({
            success: false,
            error: message,
        });
